...
typedef GeoPositionFactory = LatLng Function(double? lat, double? lng);
...
class Path<T extends LatLng> {
  final Logger _logger = Logger('universe.Path');
  ...
  LatLng _pointToPosition(Point2D point) => geoPositionFactory(
        point.x as double?,
        point.y as double?,
      );
}
