...
class TileLayerState extends State<TileLayer> with TickerProviderStateMixin {
  ...
  void didUpdateWidget(TileLayer oldWidget) {
    ...
    if (oldWidget.options.tileSize != options.tileSize ||
        oldWidget.options.retinaMode != options.retinaMode) {
      _removeAllTiles();
      _refresh();
    } else if (oldWidget.options.templateUrl != options.templateUrl) {
  ...
  Bounds _pixelBoundsToTileRange(Bounds bounds) {
    return Bounds(
      ...
      bounds.max.unscaleBy(tileSize).ceil() - const UPoint(1, 1),
    );
  }
